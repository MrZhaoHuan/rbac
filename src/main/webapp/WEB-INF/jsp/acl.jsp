    ...
    <form id="aclForm">
        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid">
            <tr>
                ...
                <td>
                    <select id="aclModuleSelectId" name="permmoduleId" data-placeholder="选择权限模块" style="width: 200px;"></select>
                </td>
            </tr>
            <tr>
...
<script id="permModuleVoListTemplate" type="x-tmpl-mustache">
<ol class="dd-list ">
    {{#permModuleVoList}}
        <li class="dd-item dd2-item aclModule-name {{displayClass}}" id="aclModule_{{id}}" href="javascript:void(0)" data-id="{{id}}">
            ...
            {{name}}
            &nbsp;
            <a class="green {{#showDownAngle}}{{/showDownAngle}}" href="javascript:void(0)" data-id="{{id}}" >
                <i class="ace-icon fa fa-angle-double-down bigger-120 sub-aclModule"></i>
            </a>
            <span style="float:right;">
                <a class="green aclModule-edit" href="javascript:void(0)" data-id="{{id}}" >
                    <i class="ace-icon fa fa-pencil bigger-100"></i>
                </a>
                &nbsp;
                <a class="red aclModule-delete" href="javascript:void(0)" data-id="{{id}}" data-name="{{name}}">
...
<tr role="row" class="acl-name odd" data-id="{{id}}"><!--even -->
    <td><a href="javascript:void(0)" class="acl-edit" data-id="{{id}}">{{name}}</a></td>
    ...
    <td>
        <div class="hidden-sm hidden-xs action-buttons">
            <a class="green acl-edit" href="javascript:void(0)" data-id="{{id}}">
                <i class="ace-icon fa fa-pencil bigger-100"></i>
            </a>
            <a class="red acl-role" href="javascript:void(0)" data-id="{{id}}">
...
<script type="text/javascript">
    $(function() {
        var permModuleVoList; // 存储树形权限模块列表
        var aclModuleMap = {}; // 存储map格式权限模块信息
        var aclMap = {}; // 存储map格式的权限点信息
        var optionStr = "";
        var lastClickAclModuleId = -1;

        var permModuleVoListTemplate = $('#permModuleVoListTemplate').html();
        Mustache.parse(permModuleVoListTemplate);
        ...
        function loadAclModuleTree() {
            $.ajax({
                url: "permModule/tree.json",
                success : function (result) {
                    if(result.code==="0") {
                        permModuleVoList = result.data;
                        var rendered = Mustache.render(permModuleVoListTemplate, {
                            permModuleVoList: result.data,
                            "showDownAngle": function () {
                                return function (text, render) {
                                    return (this.permModuleVoList && this.permModuleVoList.length > 0) ? "" : "hidden";
                                }
                            },
                        ...
                        $("#permModuleVoList").html(rendered);
        ...
        $(".aclModule-add").click(function () {
            $("#dialog-aclModule-form").dialog({
                ...
                open: function(event, ui) {
                    ...
                    recursiveRenderAclModuleSelect(permModuleVoList, 1);
                    $("#aclModuleForm")[0].reset();
                    $("#aclModuleForm [name=id]").val(""); //重置权限模块id为空
        ...
        function updateAclModule(isCreate, successCallback, failCallback) {
            $.ajax({
                url: isCreate ? "permModule/save.json" : "permModule/update.json",
        ...
        function updateAcl(isCreate, successCallback, failCallback) {
            $.ajax({
                url: isCreate ? "perm/save.json" : "perm/update.json",
        ...
        function recursiveRenderAclModuleSelect(permModuleVoList, level) {
            level = level | 0;
            if (permModuleVoList && permModuleVoList.length > 0) {
                $(permModuleVoList).each(function (i, aclModule) {
                    ...
                    if (aclModule.permModuleVoList && aclModule.permModuleVoList.length > 0) {
                        recursiveRenderAclModuleSelect(aclModule.permModuleVoList, level + 1);
                    }
                });
            }
        }

        function recursiveRenderAclModule(permModuleVoList) {
            if (permModuleVoList && permModuleVoList.length > 0) {
                $(permModuleVoList).each(function (i, aclModule) {
                    aclModuleMap[aclModule.id] = aclModule;
                    if (aclModule.permModuleVoList && aclModule.permModuleVoList.length > 0) {
                        var rendered = Mustache.render(permModuleVoListTemplate, {
                            permModuleVoList: aclModule.permModuleVoList,
                            "showDownAngle": function () {
                                return function (text, render) {
                                    return (this.permModuleVoList && this.permModuleVoList.length > 0) ? "" : "hidden";
                                }
                        ...
                        recursiveRenderAclModule(aclModule.permModuleVoList);
                    }
                })
            }
        }
        ...
        function bindAclModuleClick() {
            ...
            $(".aclModule-delete").click(function (e) {
                ...
                if (confirm("确定要删除权限模块[" + aclModuleName + "]吗?")) {
                    $.ajax({
                        url: "permModule/delete.json",
                        data: {
                            permModuleId: aclModuleId
                        },
                        success: function (result) {
                            if (result.code==="0") {
        ...
        function loadAclList(aclModuleId) {
            ...
            var url = "perm/page.json?permModuleId=" + aclModuleId;
                ...
                if (result.data.total > 0){
                    var rendered = Mustache.render(aclListTemplate, {
                        ...
                        "showAclModuleName": function () {
                            return aclModuleMap[this.permmoduleId].name;
                        },
                        "showStatus": function() {
                            return this.status == 1 ? "冻结": "有效";
                        },
                        "showType": function() {
                            return this.type == 1 ? "按钮" : (this.type == 0 ? "菜单" : "其他");
                        },
                        "bold": function() {
                            return function(text, render) {
