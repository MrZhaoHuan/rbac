    ...
    <form id="userForm">
        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid">
            ...
            <tr>
                ...
                <td><input type="text" name="email" id="userMail" value="" class="text ui-widget-content ui-corner-all"></td>
            </tr>
            <tr>
    ...
    {{#deptList}}
        <li class="dd-item dd2-item dept-name" id="dept_{{id}}" href="javascript:void(0)" data-id="{{id}}">
            ...
            <span style="float:right;">
                <a class="green dept-edit" href="javascript:void(0)" data-id="{{id}}" >
                    <i class="ace-icon fa fa-pencil bigger-100"></i>
                </a>
                &nbsp;
                <a class="red dept-delete" href="javascript:void(0)" data-id="{{id}}" data-name="{{name}}">
...
<script type="application/javascript">
    $(function() {
        ...
        function loadDeptTree() {
            $.ajax({
                url: "${pageContext.request.contextPath}/dept/tree.json",
        ...
        function recursiveRenderDept(deptList) {
            if(deptList && deptList.length > 0) {
                $(deptList).each(function (i, dept) {
                     ...
                     if (dept.deptVoList.length > 0) {
                         var rendered = Mustache.render(deptListTemplate, {deptList: dept.deptVoList});
                         $("#dept_" + dept.id).append(rendered);
                         recursiveRenderDept(dept.deptVoList);
                     }
                })
            }
        }
        ...
        function loadUserList(deptId) {
            ...
            var url = "${pageContext.request.contextPath}/user/page.json?deptId=" + deptId;
            ...
            if (result.code==="0") {
                if (result.data.total > 0){
                    var rendered = Mustache.render(userListTemplate, {
                        ...
                        "showStatus": function() {
                            return this.status == 1 ? '冻结' : (this.status == 0 ? '有效' : '删除');
                        },
                        "bold": function() {
        ...
        $(".user-add").click(function() {
            $("#dialog-user-form").dialog({
                ...
                open: function(event, ui) {
                    ...
                    var deptid = $(".btn-yellow").parent().attr("data-id");
                    if(deptid){
        ...
        function bindUserClick() {
            $(".user-acl").click(function (e) {
                ...
                $.ajax({
                    url: "${pageContext.request.contextPath}/user/acls.json",
                    data: {
                        userId: userId
                    },
                    success: function(result) {
        ...
        function recursiveRenderDeptSelect(deptList, level) {
            ...
            if (deptList && deptList.length > 0) {
                $(deptList).each(function (i, dept) {
                    ...
                    if (dept.deptVoList && dept.deptVoList.length > 0) {
                        recursiveRenderDeptSelect(dept.deptVoList, level + 1);
                    }
                });
            }
        }
        ...
        function updateUser(isCreate, successCallback, failCallback) {
            $.ajax({
                url: isCreate ? "${pageContext.request.contextPath}/user/save.json" : "${pageContext.request.contextPath}/user/update.json",
        ...
        function updateDept(isCreate, successCallback, failCallback) {
            $.ajax({
                url: isCreate ? "${pageContext.request.contextPath}/dept/save.json" : "${pageContext.request.contextPath}/dept/update.json",
