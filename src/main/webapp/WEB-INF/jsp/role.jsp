    ...
    {{#roleList}}
        <li class="dd-item dd2-item role-name" id="role_{{id}}" href="javascript:void(0)" data-id="{{id}}">
            ...
            <span style="float:right;">
                <a class="green role-edit" href="javascript:void(0)" data-id="{{id}}" >
                    <i class="ace-icon fa fa-pencil bigger-100"></i>
                </a>
                &nbsp;
                <a class="red role-delete" href="javascript:void(0)" data-id="{{id}}" data-name="{{name}}">
...
<script type="text/javascript">
    $(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.code==="0") {
                                ...
                                showMessage("删除角色[" + roleName + "]", result.message, false);
                            }
                        }
                    });
                }
        ...
        function loadRoleAcl(selectedRoleId) {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/role/roleTree.json",
                data : {
        ...
        function recursivePrepareTreeData(aclModuleList) {
            ...
            if (aclModuleList && aclModuleList.length > 0) {
                $(aclModuleList).each(function(i, aclModule) {
                    ...
                    if (aclModule.permRoleVoList && aclModule.permRoleVoList.length > 0) {
                        $(aclModule.permRoleVoList).each(function(i, acl) {
                            zTreeObj.push({
                                id: aclPrefix + acl.id,
                                pId: modulePrefix + acl.permModuleId,
                                name: acl.name + ((acl.type == 0) ? '(菜单)' : ''),
                                chkDisabled: !acl.hasPerm,
                                checked: acl.checked,
                                dataId: acl.id
                            });
                            if(acl.checked) {
                    ...
                    if ((aclModule.permModuleVoList && aclModule.permModuleVoList.length > 0) ||
                        (aclModule.permRoleVoList && aclModule.permRoleVoList.length > 0)) {
                    ...
                    recursivePrepareTreeData(aclModule.permModuleVoList);
                });
            }
        }
                ...
                buttons : {
                    "添加": function(e) {
                        ...
                        updateRole(true, function (data) {
                            ...
                            showMessage("新增角色", data.message, false);
                        })
                    },
                    "取消": function () {
                        $("#dialog-role-form").dialog("close");
        ...
        $(".saveRoleAcl").click(function (e) {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/role/changePerms.json",
                data: {
                    roleId: lastRoleId,
        ...
        function updateRole(isCreate, successCallback, failCallback) {
            if(isCreate){
                //清除form表单的元素值:角色id
                $("#roleForm [name=id]").val("");
            }

            $.ajax({
                url: isCreate ? "${pageContext.request.contextPath}/role/save.json" : "${pageContext.request.contextPath}/role/update.json",
        ...
        function loadRoleUser(selectedRoleId) {
            $.ajax({
                url: "${pageContext.request.contextPath}/role/users.json",
                data: {
                    roleId: selectedRoleId
                },
                type: 'POST',
        ...
        $(".saveRoleUser").click(function (e) {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/role/changeUsers.json",
                data: {
                    roleId: lastRoleId,
